...
static const struct mgmt_handler os_mgmt_group_handlers[] =
{
  ...
  [OS_MGMT_ID_TASKSTAT] = { os_mgmt_taskstat_read, NULL },
#endif
#ifdef CONFIG_REBOOT
...
#endif
#ifdef CONFIG_MCUMGR_GRP_OS_INFO
  [OS_MGMT_ID_INFO] = { os_mgmt_info, NULL },
#endif
};
                                   ...
                                   const struct k_thread *thread)
{
  ...
  ok = zcbor_tstr_put_lit(zse, "stksiz") && zcbor_uint64_put(zse, stack_size)
                                     ...
                                     const struct k_thread *thread)
{
  ...
  ok = zcbor_tstr_put_lit(zse, "runtime") && zcbor_uint32_put(zse, 0);
#endif
...
static inline bool os_mgmt_taskstat_encode_unsupported(zcbor_state_t *zse)
{
  ...
  if (!IS_ENABLED(CONFIG_MCUMGR_GRP_OS_TASKSTAT_ONLY_SUPPORTED_STATS))
    {
      ok = zcbor_tstr_put_lit(zse, "cswcnt") && zcbor_uint32_put(zse, 0)
...
static inline bool
os_mgmt_taskstat_encode_priority(zcbor_state_t *zse,
                                 const struct k_thread *thread)
{
  return (
      zcbor_tstr_put_lit(zse, "prio")
...
static void os_mgmt_taskstat_encode_one(const struct k_thread *thread,
                                        void *user_data)
{
  /*
  ...
  struct thread_iterator_info *iterator_ctx
      = (struct thread_iterator_info *)user_data;

  if (iterator_ctx->ok == true)
    {
      iterator_ctx->ok
          = os_mgmt_taskstat_encode_thread_name(
                iterator_ctx->zse, iterator_ctx->thread_idx, thread)
            && zcbor_map_start_encode(iterator_ctx->zse,
                                      TASKSTAT_COLUMNS_MAX)
            ...
            && os_mgmt_taskstat_encode_runtime_info(iterator_ctx->zse,
                                                    thread)
...
static int os_mgmt_taskstat_read(struct smp_streamer *ctxt)
{
  ...
  if (!iterator_ctx.ok
      || !zcbor_map_end_encode(
          zse, CONFIG_MCUMGR_GRP_OS_TASKSTAT_MAX_NUM_THREADS))
    {
...
static int os_mgmt_reset(struct smp_streamer *ctxt)
{
  ...
  status = mgmt_callback_notify(MGMT_EVT_OP_OS_MGMT_RESET, NULL, 0, &ret_rc,
                                &ret_group);
...
static int os_mgmt_info(struct smp_streamer *ctxt)
{
  ...
  if (zcbor_map_decode_bulk(zsd, fs_info_decode, ARRAY_SIZE(fs_info_decode),
                            &decoded))
    {
  ...
  if (valid_formats != format.len)
    {
      ...
      bool ok = smp_add_cmd_ret(zse, MGMT_GROUP_ID_OS,
  ...
  /* If custom_os_name is not set (by extension code) then return the default
   * OS name of Zephyr
   */
  if (format_bitmask & OS_MGMT_INFO_FORMAT_OPERATING_SYSTEM
      && custom_os_name == false)
    {
      rc = snprintf(&output[output_length], (sizeof(output) - output_length),
  ...
  status
      = mgmt_callback_notify(MGMT_EVT_OP_OS_MGMT_INFO_APPEND, &append_data,
