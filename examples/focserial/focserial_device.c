/****************************************************************************
 * apps/examples/focserial/foc_device.c
 *
...
int foc_device_init(FAR struct foc_device_s *dev, int id)
{
  ...
  sprintf(devpath, "%s%d", CONFIG_EXAMPLES_FOCSERIAL_FOC_DEVPATH, id);
  ...
  cfg.pwm_freq      = (CONFIG_EXAMPLES_FOCSERIAL_PWM_FREQ);
  cfg.notifier_freq = (CONFIG_EXAMPLES_FOCSERIAL_NOTIFIER_FREQ);
