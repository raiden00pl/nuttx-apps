...
#include <assert.h>
#include <mcumgr/mgmt/mgmt.h>
#include <mcumgr/smp/smp.h>
                                ...
                                struct net_buf *nb)
{
  ...
  cnr->nb = nb;
  zcbor_new_decode_state(cnr->zs, ARRAY_SIZE(cnr->zs), new_ptr, cnr->nb->len,
                         1);
}

static void cbor_nb_writer_init(struct cbor_nb_writer *cnw,
                                struct net_buf *nb)
{
  net_buf_reset(nb);
  cnw->nb      = nb;
...
static void smp_make_rsp_hdr(const struct smp_hdr *req_hdr,
                             struct smp_hdr *rsp_hdr, size_t len)
{
...
static inline int smp_write_hdr(struct smp_streamer *streamer,
                                const struct smp_hdr *src_hdr)
{
  memcpy(streamer->writer->nb->data, src_hdr, sizeof(*src_hdr));
  return 0;
}

static int smp_build_err_rsp(struct smp_streamer *streamer,
                             const struct smp_hdr *req_hdr, int status,
                             const char *rc_rsn)
{
...
static int smp_handle_single_payload(struct smp_streamer *cbuf,
                                     const struct smp_hdr *req_hdr,
                                     bool *handler_found)
{
  ...
  if (handler_fn)
    {
      *handler_found = true;
      ...
      (void)mgmt_callback_notify(MGMT_EVT_OP_CMD_RECV, &cmd_recv,
                                 sizeof(cmd_recv), &ret_rc, &ret_group);
#endif
 ...
 * supplied streamer but not transmitted.  On failure, no error response gets
 * written; the caller is expected to build an error response from the return
 * code.
 *
 * @param streamer	The SMP streamer to use for reading the request and
...
static int smp_handle_single_req(struct smp_streamer *streamer,
                                 const struct smp_hdr *req_hdr,
 ...
 *   supplied buffers.
 *
 * @param streamer	The SMP streamer for building and transmitting the
 *response.
 * @param req_hdr	The header of the request which elicited the error.
 * @param req		The buffer holding the request.
 * @param rsp		The buffer holding the response, or NULL if none was
 *allocated.
 * @param status	The status to indicate in the error response.
 * @param rsn		The text explanation to @status encoded as "rsn" into
 *CBOR response.
 *
 ****************************************************************************/

static void smp_on_err(struct smp_streamer *streamer,
                       const struct smp_hdr *req_hdr, void *req, void *rsp,
                       int status, const char *rsn)
{
  int rc;
 ...
 *   the end.  Each response is sent individually in its own packet.  If a
 *   request elicits an error response, processing of the packet is aborted.
 *   This function consumes the supplied request buffer regardless of the
 *   outcome.
...
bool smp_add_cmd_ret(zcbor_state_t *zse, uint16_t group, uint16_t ret)
{
  ...
  if (ret != 0)
    {
#ifdef CONFIG_MCUMGR_SMP_SUPPORT_ORIGINAL_PROTOCOL
      struct cbor_nb_writer *container
          = CONTAINER_OF(zse, struct cbor_nb_writer, zs);

      container->error_group = group;
      container->error_ret   = ret;
#endif

      ok = zcbor_tstr_put_lit(zse, "ret") && zcbor_map_start_encode(zse, 2)
